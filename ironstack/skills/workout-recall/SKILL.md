...
- `workout-sessions` — one doc per session: `date`, `start_time`, `time_of_day`, `location.name` (+ optional `location.geo`, `location.travel`), `environment` (temp_f, humidity_pct, conditions), `program` (name, block, day, total_days), `totals` (tonnage_lb, sets, working_sets, reps, exercises), `avg_working_rpe`, `wrap_up` (text; `wrap_up_semantic` where ELSER is enabled), `watch_items`.
- `workout-sets` — one doc per set: exercise (name, slug, category prep|main|accessory, equipment), `set_number`, `set_type` (prep|working), `weight_lb`, `reps`, `rep_unit` (reps|walks|seconds), `distance_ft`, `load_type`, `rpe`, `volume_lb`, `est_e1rm`, `gear`, `notes`, `tags` — plus denormalized `session_id`, `date`, `time_of_day`, `location.name`, `program.day`.
- `workout-notes` — one doc per mindset/observation note: `phase` (pre|prep|exercise|wrap-up), `exercise`, `order`, `text` (plus `text_semantic` where ELSER is enabled), `tags` — with the same denormalized session context.
