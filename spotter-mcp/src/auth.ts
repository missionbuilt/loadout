/**
 * OAuth handler for The Spotter MCP server.
 *
 * Implements the user-facing side of MCP OAuth 2.1 with Google as the
 * upstream identity provider. Wrapped by @cloudflare/workers-oauth-provider
 * which handles the OAuth protocol endpoints (.well-known discovery, /token,
 * /register, etc.) — this handler is responsible for /authorize and the
 * Google round-trip, plus the public landing/preview/health routes.
 *
 * Flow:
 ...
 * Routes to OAuth flow handlers, public pages, or 404.
 */
export const authHandler = {
  async fetch(request: Request, env: AuthEnv, ctx: ExecutionContext): Promise<Response> {
    const url = new URL(request.url);

    // OAuth flow — user is being asked to authenticate
    if (url.pathname === "/authorize") {
      return handleAuthorize(request, env);
    }

    // Google callback after user signs in
    ...
    // Favicon — served at every common path Claude Desktop / browsers /
    // OS-level fetchers might probe, so nothing falls back to an upstream icon.
    if (
      ...
      // and a cream bar on the warm-dark base. Matches missionbuilt.io exactly.
  ...
  <rect x="20" y="13" width="6" height="6" fill="#a8211a"/>
    ...
    // Web app manifest — referenced by some clients, points back at the SVG.
    ...
    if (url.pathname === "/health") {
      return new Response(
        JSON.stringify(
          {
            ok: true,
            server: "the-spotter-mcp",
            server_version: SERVER_VERSION,
            spotter_version: SPOTTER_VERSION,
            auth: "oauth-google",
...
/**
 * /authorize — called when Claude Desktop redirects the user here to begin
 * authentication. We parse the OAuth request, store it in state, and forward
 * the user to Google for sign-in.
 */
async function handleAuthorize(request: Request, env: AuthEnv): Promise<Response> {
  const oauthReqInfo = await env.OAUTH_PROVIDER.parseAuthRequest(request);

  // We encode the OAuth provider's request info into the state parameter
  // we send to Google. Google echoes it back to us at /google/callback,
  // and we use it to complete the authorization with the OAuth provider.
...
/**
 * /google/callback — Google redirects the user here after sign-in with a
 * one-time authorization code. We exchange it for the user's identity, then
 * complete the OAuth flow with our provider library.
 */
async function handleGoogleCallback(request: Request, env: AuthEnv): Promise<Response> {
  ...
  // Fetch the user's identity from Google
  ...
  // Complete the OAuth flow with our provider. This issues an MCP auth code
  // and gives us the URL to redirect the user back to (which is Claude Desktop's
  // redirect URI).
...
/**
 * Renders a branded error page in the Mission Built design system.
 */
...
<head>
  ...
  <title>The Spotter · Sign-in error</title>
...
<body>
  <div class="mb-container">
    ...
    <p class="mb-p" style="margin-top: 2rem;"><a class="mb-link" href="/">← Back to The Spotter</a></p>
    <div class="mb-footer">
