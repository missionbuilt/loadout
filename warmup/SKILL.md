...
description: >
  ...
  Trigger phrases: "warmup", "morning brief", "run the warmup",
...
**Trigger phrases include:** *"warmup"*, *"morning brief"*, *"run the warmup"*,
*"give me my warmup"*, *"what's in the brief today"*, *"warmup setup"*,
*"set up the warmup"*, *"configure the warmup"*, *"warmup config"*,
*"add [source] to my warmup"*, *"remove [source] from warmup"*,
...
Ask these four questions. One at a time, not as a list dump.

1. *"What industry or sector is your organization in?"*
   Accept open-form. Map to the Sector Sources table below. Common answers:
   Healthcare, Financial Services, Energy/Utilities, Government/Public Sector,
   Technology, Retail/Consumer, Manufacturing/OT, Critical Infrastructure.
   If unclear, ask a clarifying follow-up.

2. *"What's your company name? (Optional — used only to tailor threat actor
   context. Stored only in your local WARMUP.md, never sent anywhere.)"*
   Accept or skip gracefully.

3. *"What region are you primarily operating in?"*
   Accept open-form. Common answers: United States, European Union, APAC,
   Latin America, Global. Used to prioritize regionally-relevant advisories
   and regulatory news.

4. *"Any specific vendors or competitors you want to track?"*
   Accept a comma-separated list (e.g., "Palo Alto, CrowdStrike, Wiz").
   These will be tracked in the Industry Intel section.

5. *"Last one — anything you want to stay current on outside of work? Sports,
   markets, a hobby, a specific team? Totally optional — just add it to the
   end of your brief as a Special Interests section."*
   Accept open-form. Examples: "SEC football, NBA", "Formula 1", "Pittsburgh
   Steelers", "bourbon releases", "Formula 1 and venture capital".
...
Ask: *"Describe what you want in your morning brief. Be specific — topics,
...
**Trigger:** "warmup", "morning brief", "run the warmup", "give me my warmup",
"what's in the brief today"
...
1. Call `list_artifacts` to find the artifact id (typically `the-warmup`).
2. Read the artifact's current HTML from the path returned by `list_artifacts`.
3. Replace ONLY the `<script id="warmup-data">` block with the new `window.WARMUP_DATA = {...};`. Do NOT touch any other part of the file.
4. Write the modified HTML to `warmup-artifact.html` in the workspace folder.
5. Call `update_artifact` with `id` and `html_path` pointing to the written file.

Output cost: ~5KB (content data only). The CSS, JS, PDF builder, profile card, section collapse, accessibility markup — all stay exactly as-is.
...
1. Read `warmup/warmup-template.html` — this is the canonical engine. Do NOT build from scratch.
2. Replace the `<script id="warmup-data">` block with the generated `WARMUP_DATA` for this run.
