...
> **Superseded:** This is a point-in-time snapshot from 2026-05-20 — the code has moved since. `approach_get_template` no longer returns the filled HTML in a single call; like the other `*_get_template` tools, it now paginates in 900-line chunks.
...
| `approach_get_template` | ~10K (JSON) | ~150K (HTML) | Largest response; paginated in 900-line chunks like the other `*_get_template` tools |
